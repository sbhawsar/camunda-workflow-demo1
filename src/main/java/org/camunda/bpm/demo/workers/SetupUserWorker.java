...
public class SetupUserWorker {
    ...
    private static void subscribeAndExecuteUserWorkflow(ExternalTaskClient client) {
        ...
        .handler((externalTask, externalTaskService) -> {
            ...
            String authToken = externalTask.getVariable("authToken");
            
            CreateUserWorker.createUser(username, email, authToken);

            // Get a process variable
            LOGGER.info(
                    "Create user is invoked with inputs available .... .TODO: No inputs are given currently");

            String channel = externalTask.getVariable("channel");
            if (channel == null) {
                channel = "email";
            }
